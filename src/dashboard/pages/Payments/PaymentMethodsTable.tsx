...
const methods: { id: PaymentMethodId; label: string; action: 'toggle' | 'setup'; setupLabel?: string }[] = [
...
export default function PaymentMethodsTable({ onSetupMethod }: Props) {
  const { data } = usePaymentMethods()
  const save = useSavePaymentMethods()

  const config: PaymentMethodsConfig = data?.methods ?? {}

  const isEnabled = (id: PaymentMethodId) => !!config[id]?.enabled

  const toggle = (id: PaymentMethodId) => {
    const updated: PaymentMethodsConfig = {
      ...config,
      [id]: { ...config[id], enabled: !isEnabled(id) },
    }
    save.mutate(updated)
  }
  ...
  return (
    <div className="bg-white rounded-xl border border-slate-100 overflow-hidden">
      ...
      {methods.map(({ id, label, action, setupLabel }, i) => (
        <div
          key={id}
          className={`grid grid-cols-[1fr_auto] sm:grid-cols-3 items-center gap-3 px-5 sm:px-6 py-4 ${i < methods.length - 1 ? 'border-b border-slate-100' : ''}`}
        >
          ...
          <span className={`inline-flex w-fit px-2.5 py-1 rounded-md text-[11px] font-semibold justify-self-end sm:justify-self-start ${
            isEnabled(id) ? 'bg-emerald-50 text-emerald-600' : 'bg-slate-100 text-slate-400'
          }`}>
          ...
          {action === 'toggle' ? (
            <div className="col-span-2 sm:col-span-1">
              <Toggle on={isEnabled(id)} onToggle={() => toggle(id)} />
            </div>
          ) : (
            <button
            ...
            >
              {isEnabled(id) ? 'Manage account' : setupLabel}
            </button>
          )}
        </div>
      ))}
