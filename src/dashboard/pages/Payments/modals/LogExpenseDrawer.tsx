...
import { useLogExpense } from '../../../../hooks/usePayments'
...
export default function LogExpenseDrawer({ open, onClose }: Props) {
  ...
  const [form, setForm] = useState({ title: '', amount: '', category: 'Inventory', date: '', note: '' })
  ...
  const handleSubmit = () => {
    ...
    logExpense.mutate(
      { title: form.title, amount: form.amount, category: form.category, date: form.date || undefined, note: form.note || undefined },
            ...
            <div className="px-6 py-4 border-t border-slate-100 shrink-0">
              {done ? (
                ...
                <div className="flex gap-2">
                  ...
                  <button onClick={handleSubmit} disabled={logExpense.isPending} className="flex-1 py-3 rounded-xl bg-slate-900 text-white text-[13px] font-semibold hover:bg-slate-700 transition-colors disabled:opacity-60 flex items-center justify-center gap-2">
                    {logExpense.isPending && <span className="w-3.5 h-3.5 border-2 border-white/30 border-t-white rounded-full animate-spin" />}
