...
export default function ShippingZonesEmpty({ onAddZone }: Props) {
  return (
    <div className="bg-white rounded-xl border border-slate-100 flex flex-col items-center justify-center py-20 gap-3">
      ...
      <p className="text-[14px] font-semibold text-slate-800">No shipping options set up yet</p>
      <p className="text-[12px] text-slate-400 text-center max-w-xs leading-relaxed">
        Make it easy for your customers to receive their orders by setting up shipping methods. Choose between flat rates, local delivery, or pickup options.
      </p>
      <button
        onClick={onAddZone}
        className="flex items-center gap-1.5 mt-2 px-4 py-2 rounded-lg border border-slate-200 text-[13px] font-medium text-slate-600 hover:bg-slate-50 transition-colors"
      >
