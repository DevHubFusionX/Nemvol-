...
export default function LaunchStorefrontModal({ open, onClose, onPublished }: Props) {
  ...
  const [form, setForm] = useState({ storeName: '', headline: '', notice: '', whatsapp: '', instagram: '' })
  ...
  const handlePublish = () => {
    if (!form.storeName.trim()) { setErrors({ storeName: 'Store name is required' }); return }
    setErrors({})
    if (submitting.current) return
    submitting.current = true
    update.mutate(
      { name: form.storeName, whatsapp: form.whatsapp || undefined, published: 'true' },
      {
        onSuccess: () => {
          setDone(true)
          onPublished()
          ...
          >
            ...
            <div className="flex-1 overflow-y-auto px-6 pt-12 pb-4">
              <AnimatePresence mode="wait">
                {done ? (
                  <motion.div key="done" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="flex flex-col items-center text-center py-8 gap-3">
                  ...
                  <motion.div key="form" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-5">
                    ...
                    <div>
                      <label className="block text-[13px] font-semibold text-slate-800 mb-1.5">
                        Store Notice{' '}
                        <span className="text-[11px] font-normal text-slate-400">(appears at the top of your website)</span>
                      </label>
                      <textarea rows={3} value={form.notice} onChange={set('notice')} placeholder="Inform your customers about any notice about your store" className={`${inputCls} resize-none`} />
                    </div>

                    <div className="rounded-xl border border-slate-100 p-4 space-y-3">
                      <p className="text-[13px] font-semibold text-slate-800">Social Links</p>
                      <input type="text" value={form.whatsapp} onChange={set('whatsapp')} placeholder="WhatsApp number or link" className={inputCls} />
                      <input type="text" value={form.instagram} onChange={set('instagram')} placeholder="Instagram handle or link" className={inputCls} />
                    </div>
                  </motion.div>
                )}
              </AnimatePresence>
            ...
            <div className="px-6 py-4 border-t border-slate-100 shrink-0">
              {done ? (
                ...
                <button onClick={handlePublish} disabled={update.isPending} className="w-full py-3 rounded-xl bg-slate-900 text-white text-[13px] font-semibold hover:bg-slate-700 transition-colors disabled:opacity-60 flex items-center justify-center gap-2">
                  {update.isPending && <span className="w-3.5 h-3.5 border-2 border-white/30 border-t-white rounded-full animate-spin" />}
