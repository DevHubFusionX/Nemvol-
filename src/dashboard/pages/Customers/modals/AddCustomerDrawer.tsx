...
const segments = ['New', 'Active', 'VIP', 'At Risk'];
...
export default function AddCustomerDrawer({ open, onClose }: Props) {
  ...
  const reset = () => {
    setDone(false);
    setErrors({});
    setForm({ name: '', email: '', phone: '', segment: 'New', address: '', note: '' });
  };
              ...
              <AnimatePresence mode="wait">
                {done ? (
                  ...
                  >
                    ...
                    <p className="text-[15px] font-bold text-slate-900">Customer added!</p>
                    <p className="text-[13px] text-slate-400 max-w-xs leading-relaxed">
                      <span className="font-semibold text-slate-700">{form.name}</span> has been added to your customer list.
                    </p>
                  </motion.div>
                ) : (
                  <motion.div key="form" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-5">
                    <div>
                      <h2 className="text-[17px] font-bold text-slate-900">Add Customer</h2>
                      <p className="text-[12px] text-slate-400 mt-0.5">Create a customer profile for orders and follow-ups.</p>
                    </div>

                    <div className="rounded-xl border border-slate-100 p-4 space-y-3">
                      <p className="text-[13px] font-semibold text-slate-800">Customer Info</p>
                      <div>
                        <input type="text" placeholder="Full name *" value={form.name} onChange={set('name')} className={inputCls} />
                    ...
                    <div>
                      <label className="block text-[13px] font-semibold text-slate-800 mb-2">Customer Segment</label>
                      <div className="flex flex-wrap gap-2">
                        {segments.map(segment => (
                          <button
                            key={segment}
                            type="button"
                            onClick={() => setForm(p => ({ ...p, segment }))}
                    ...
                    <div>
                      <label className="block text-[13px] font-semibold text-slate-800 mb-1.5">
                        Address <span className="text-[11px] font-normal text-slate-400">(optional)</span>
                      </label>
                      <input type="text" placeholder="Delivery address" value={form.address} onChange={set('address')} className={inputCls} />
                    </div>
                    ...
                    <div>
                      ...
                      <textarea rows={3} placeholder="Internal note about this customer..." value={form.note} onChange={set('note')} className={`${inputCls} resize-none`} />
                    </div>
                  </motion.div>
                )}
              </AnimatePresence>
